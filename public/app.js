document.addEventListener('DOMContentLoaded', () => {
    // 1. Initialize Firebase from global config (no fetch)
    ...
    sessionsList.addEventListener('click', (e) => {
        if (e.target.classList.contains('details-btn')) {
            ...
            // Fetch single session via direct DB
    ...
    saveSessionBtn.addEventListener('click', () => {
        ...
        const newSession = {
            ...
            images: {
                home_screen: imagesData.home_screen.map(img => ({ ...img,
                    clickCount: 0
                })),
                game_over_screen: { ...imagesData.game_over_screen,
                    clickCount: 0
                }
            },
        };

        database.ref('sessions').push(newSession).then(() => {
            resetCreateSessionForm();
            showView(sessionsView);
            loadSessions();
        });
    ...
    const loadPlayerAnalytics = async () => {
        try {
            // Replace backend call with client-side logic
            const jungleJumperSnapshot = await database.ref('Jungle Jumper').once('value');
            const sandsOfCairoSnapshot = await database.ref('Sands of Cairo').once('value');

            const allPlays = [];
            if (jungleJumperSnapshot.exists()) {
                jungleJumperSnapshot.forEach(child => allPlays.push(child.val()));
            }
            if (sandsOfCairoSnapshot.exists()) {
                sandsOfCairoSnapshot.forEach(child => allPlays.push(child.val()));
            }
            ...
            Object.values(uniquePlayers).forEach(player => {
                if (player.playCount > 1) returningPlayers += 1;
                if (player.playCount > highestPlayCount) highestPlayCount = player.playCount;
            });
